...
`starting_positions` is signed in PostgreSQL (`-9007199254740991` through
`9007199254740991`) so an overdrawn opening can be represented. The shipped UI
restricts entry to zero or positive; the database still accepts a negative row
for forward compatibility, but the onboarding form only sends
`p_balance_cents >= 0` and the detail page renders whatever signed value the
row holds with `formatSignedCents` (U+2212 for negatives). `effective_on` is
the balance at the opening of that date; the UI fixes it to the device-local
`YYYY-MM-DD` (`getLocalTodayIsoDate`, no UTC conversion) and explains that
transactions on the same date apply after it. The row is immutable in this
release: `select` and `insert` are granted to `authenticated`, `update` and
`delete` are not, and `initialize_starting_position` is `security invoker`
with `search_path = ''`.
...
## Authentication and financial-workspace guard
...
so a password-recovery session cannot open `/app` or `/onboarding`.
...
PostgreSQL grants and RLS still authorize financial data. A logout or
authenticated user ID change clears the TanStack Query cache before the next
identity renders. Refreshes for the same user keep the cache.

`src/routes/_authenticated._positioned.tsx` is a second pathless guard below
`_authenticated`. It fetches `startingPositionQueryOptions(userId)` with
`queryClient.fetchQuery` (the current `QueryClient` API; `ensureQueryData` and
`fetchQuery` are deprecated in the installed TanStack Query version). If the
read returns `null` it redirects to `/onboarding`; if it rejects it renders the
retry/logout error state and never masquerades as `null`. A successful row is
returned in route context as `startingPosition`. `src/routes/_authenticated.onboarding.tsx`
does the inverse: it redirects to `/app` when a row already exists and otherwise
renders `StartingPositionPage`. Both guards use `isRedirect` to rethrow
redirects.

The financial route tree is:
...
/_authenticated
  ...
  /_positioned
    ...
    /app/starting-position?month=YYYY-MM&notice=already-saved
```

`_positioned` owns the three financial routes; `onboarding` is its sibling so the
missing-position redirect does not trap itself. `src/routeTree.gen.ts` is
generated by the TanStack Router Vite plugin and is not hand-edited.

## Starting-position cache and conflict recovery

`src/features/starting-position/starting-position-service.ts` owns the
owner-scoped read (`select('*').abortSignal(signal).maybeSingle()`, no
`eq(user_id)`) and `initialize_starting_position(p_balance_cents,
p_effective_on)`. `starting-position-queries.ts` keys the row as
`['starting-position', userId]` with `staleTime: q => q.state.data ? 'static' : 0`
— a saved row never changes in this release, while `null` is immediately stale
so another tab's insert is not missed. `starting-position-mutations.ts` writes
the RPC's authoritative return value with `setQueryData` for the exact user key,
then the route invalidates and replaces the URL with `/app`. No transaction
month is invalidated; the opening position is not a transaction.

`StartingPositionPage` keeps the draft and review values after every failure.
An identical RPC retry returns the same row (idempotent). A `23505`
`starting_position_already_exists` with different values is treated as a
conflict: the page refetches the authoritative row, seeds the cache, and the
onboarding route navigates to `/app/starting-position?notice=already-saved`,
which renders the saved `R$` value and `effective_on` and explains that the
rejected draft was not saved. The error mapper `getStartingPositionErrorCopy`
and `isStartingPositionAlreadyExists` select copy by `${code}:${message}` and
never surface provider details.

`StartingPositionDetailPage` prefers the cached row already fetched by
`_positioned` and otherwise uses `useQuery(startingPositionQueryOptions)`; the
static `staleTime` guarantees no second fetch when the row is already in memory.
The workspace exposes the detail via a `44×44` `Ponto de partida` link inside
the existing account capsule, next to `Sair` and `Adicionar`, with `shrink-0`
so the month buttons keep `44×44` at `375×667`.

No financial draft is persisted in `localStorage`, Zustand, TanStack Query
outside memory, or the service worker.
...
Observed on 2026-08-28 on `feat/starting-position` `8d148e1`: `supabase
db reset` applied `20260825010000`, `20260825020000`, `20260827010000`, and
`20260827020000`, `77` pgTAP checks across `2` files, `database.types.ts`
reproducible (`git diff --exit-code` clean), `0` `oxlint` warnings, `292`
Vitest tests across `35` files, `56` Playwright cases (`auth`, `bootstrap`,
`onboarding`, `transactions-read/create/edit/delete`) on `mobile-chromium`
(`375×667`) and `desktop-chromium` (`1280×800`) against local Supabase, and a
production build with `50` precached entries and `workbox.runtimeCaching: []`
(`vite.config.ts:58`) so Supabase, Auth, and financial responses are never
cached. Build logs contain no access token, `service_role` key, or financial
payload. The onboarding review, `already-saved` detail, and workspace `Ponto de
partida` action were observed in both viewports and color schemes with no
horizontal overflow and every control `44×44`.
